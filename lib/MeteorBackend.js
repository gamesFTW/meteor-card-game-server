...
Meteor.methods({
    ...
    playCard: function(gameId, id, position) {
        ...
        if (_.contains(['creature', 'area'], card.type) && isCastable && isHaveMana) {
    ...
    playCardAsSpell: function(gameId, playedCardId, targetCardId) {
        ...
        var untappedCardInManaPool = MeteorApp.CardsInGame.findOne({
            gameId: playedCard.gameId,
            ownerId: playedCard.ownerId,
            cardGroup: GroupName.MANA_POOL,
            tapped: false
        });
        ...
        var isHaveMana = untappedCardInManaPool != undefined;

        if (fromTable && playedCard.attachable && isTargetCardTypeCreature && isHaveMana) {
            ...
            // Tap mana
            Meteor.call('tapCard', untappedCardInManaPool.gameId, untappedCardInManaPool._id)
        }
    },
