...
Meteor.methods({
    ...
    addEndOfTurnEvent: function(gameId, playerId) {
        ...
        // Сохраняем кол-во ходов игры в базу
        let game = MeteorApp.Games.findOne(gameId);
        let lastPlayerId = game.type == 'solo' ? game.playerId2 : game.playerId4;
        
        let gameTurnNumber = MeteorApp.Actions.find({
            gameId: gameId,
            type: 'Backend:endOfPlayerTurn',
            'params.playerId': lastPlayerId 
        }).count();
        
        let isNewTurn = game.turnNumber != gameTurnNumber;
